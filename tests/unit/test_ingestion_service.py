...
Covers:
  ...
  - ingest_batch(): never raises — per-file failures appear as FAILED events
...
def fake_path(tmp_path: Path) -> Path:
    """A real file on disk so parser.parse() can be invoked on it."""
    p = tmp_path / "notes.fake"
...
class TestIngestHappyPath:
    def test_calls_each_observer_once(
        ...
        service.ingest(fake_path, user_id)
    ...
    def test_user_id_is_stamped(
        ...
        obs = _make_observer()
        service.subscribe(obs)
        event = service.ingest(fake_path, user_id)
...
class TestIngestObserverFailures:
    ...
    def test_document_not_marked_ready_on_failure(
        ...
        # We can't pull the document out of the raised exception cleanly,
        # but we know the observer captured the same event — and event.fail()
...
class TestIngestBatch:
    ...
    def test_processes_all_paths(
        ...
        paths: list[Path] = []
        for i in range(5):
            p = tmp_path / f"file{i}.fake"
            p.write_text(f"content {i}", encoding="utf-8")
            paths.append(p)

        results = service.ingest_batch(paths, user_id)
    ...
    def test_preserves_order(
        ...
        for path, event in zip(paths, results):
            assert event.document.filename == path.name
    ...
    def test_per_file_failure_does_not_abort_batch(
        ...
        """Two good files + one with an unsupported extension → 3 events back, 1 FAILED."""
        with IngestionService(registry) as svc:
            svc.subscribe(_make_observer())

            good1 = tmp_path / "a.fake"
            good1.write_text("a", encoding="utf-8")
            bad = tmp_path / "b.pdf"  # no parser registered for .pdf
            bad.write_text("b", encoding="utf-8")
            good2 = tmp_path / "c.fake"
            good2.write_text("c", encoding="utf-8")

            results = svc.ingest_batch([good1, bad, good2], user_id)
    ...
    def test_batch_does_not_raise_on_observer_failure(
        ...
        with IngestionService(registry) as svc:
            ...
            # No raise.
            results = svc.ingest_batch(paths, user_id)
